...
//                            }
            ...
            String[] rawUUID = queryParam.split(RemoteSessionConstants.OAuthTokenValidator
                    .QUERY_KEY_VALUE_SEPERATOR);
            if (rawUUID.length != 2) {
                log.warn("Invalid query string [" + queryParam + "] passed in.");
            }
            if (rawUUID[0].equals(RemoteSessionConstants.OAuthTokenValidator.TOKEN_IDENTIFIER)) {
                uuid = rawUUID[1];
            }
            if(uuid != null && uuid.isEmpty()){
                log.error("Could not find a UUID related to the remote session");
            } else {
                log.error("The UUID ID that device brings " + uuid);
                ...
                if(tenantDomain == null || tenantDomain.isEmpty()){
                    ...
                    //initializeDeviceSession(session, "carbon.super", deviceType, deviceId, operationId, uuid);
                } else {
                    // create new device session
    ...
    private void initializeClientSession(Session session, String tenantDomain, String deviceType, String deviceId) throws RemoteSessionManagementException,
        ...
        log.error("The UUID ID that client creates " + uuidToValidateDevice);
        ...
        if (activeSession != null && activeSession.getMySession().isOpen() && activeSession
            ...
            if (activeSession == null) {
                ...
                log.error("UUID " + uuidToTenantMap + " is generated against the tenant : " + tenantDomain);
                log.error("UUID " + uuidToValidateDevice + " is generated against the tenant : " +
